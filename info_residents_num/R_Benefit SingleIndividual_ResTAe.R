...
TAeInfo=seq(0.1, 1, 0.1); TAeInfo # Infomrationslevels der residenten Population (Szenariem)
...
for (i in 1:ndata) {
  dummy = numeric()
  for (r in 1:repli) {
    filename = paste0("../info_residents_ibm/results/output_exp",i,"_rep",r,".txt")
    file = read.table(filename,header=T)
    dummy[r] = mean(file$emi_TAe)
  }
...
{
  filename = paste0("../info_residents_ibm/results/popsizes_I",TAeInfo[i],".txt")
...
M_resDIE=numeric(ndata)   #mean emigration for resident DIE population (last generation) -> from data
M_bestVT=numeric(ndata)   #find INDEX of threhold value that generates optimal fitness
M_bestgVT=numeric(ndata)  #dito for geometric mean
M_bestemi_DD=numeric(ndata) # ... and corresponding realized emigration prob.
M_fitMax_DD=numeric(ndata) # max. arith fitness
M_gfitMax_DD=numeric(ndata) # max. geom fitness
M_mfit_DI=numeric(ndata) # fitness of non-informed invader
M_gfit_DI=numeric(ndata) # dito geom. mean
M_geo0_DI=numeric(ndata) # geom. fitness never emigrating invader
M_geo1_DI=numeric(ndata) # geom. fitness always emigrating invader
M_vemip_DD=matrix(nrow=length(vCT), ncol=ndata) # emigration prob for full-informed for given CTs
M_vmfit_DD=matrix(nrow=length(vCT), ncol=ndata) # dito fitness
M_vgmfit_DD=matrix(nrow=length(vCT), ncol=ndata)# dito geom. fitness
...
for(i in 1:ndata)
{
  ...
  M_resDIE[i]=resDIE  #mean emigration for resident DIE population (last generation)
  M_bestVT[i]=which(vmfit_DD==max(vmfit_DD))[1] # find index for threhold value CT that generates optimal fitness
  M_bestgVT[i]=which(vgmfit_DD==max(vgmfit_DD))[1]; # dito for geometric mean
  M_fitMax_DD[i]=vmfit_DD[M_bestVT[i]] # max fitness for arithmetic mean
  M_gfitMax_DD[i]=vgmfit_DD[M_bestgVT[i]] #max fitness  for geometric mean
  M_mfit_DI[i]=mfit_DI
  M_gfit_DI[i]=gfit_DI
  M_geo0_DI[i]=geo0_DI
  M_geo1_DI[i]=geo1_DI
  M_vemip_DD[,i]=vemip_DD # threshold dependen emigration prob of CT
  M_bestemi_DD[i]=vemip_DD[M_bestVT[i]]
  M_vmfit_DD[,i]=vmfit_DD
  M_vgmfit_DD[,i]=vgmfit_DD
...
M_bestCT=vCT[M_bestVT]
M_bestgCT=vCT[M_bestgVT]

##################################### end data processing ##################################################

save.image(file="processed_data.RDat")
